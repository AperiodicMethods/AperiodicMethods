...
def plot_ap_sims(sims_exp, sims_comb, measure=None, expected=None,
                 avg_func=np.mean, var_func=np.std, ylim=None, legend_loc=None,
                 x_exps=EXPS, figsize=(6, 5)):
    """Plot simulation results across aperiodic parameters."""

    _, ax = plt.subplots(figsize=figsize)

    if expected is not None:
        plot_lines(np.abs(x_exps), expected, color='k', linestyle='--', label='Expected')

    plot_lines(np.abs(x_exps), avg_func(sims_exp, 1), var_func(sims_exp, 1),
               color=COLORS['AP'], label='Aperiodic', ax=ax)
    plot_lines(np.abs(x_exps), avg_func(sims_comb, 1), var_func(sims_comb, 1),
               color=COLORS['COMB'], label='Combined', ax=ax)
...
def plot_pe_sims(sims_freq, sims_pow, measure=None, expected=None,
                 avg_func=np.mean, var_func=np.std, ylim=None, legend_loc=None,
                 x_freqs=FREQS, x_pows=POWERS,figsize=(6, 5)):
    """Plot simulation results across periodic parameters."""

    _, ax = plt.subplots(figsize=figsize)

    plot_lines(x_freqs, avg_func(sims_freq, 1), var_func(sims_freq, 1),
               color=COLORS['CF'], xlabel='Oscillation Frequency', ylabel=measure, ax=ax)

    ax2 = ax.twiny()
    plot_lines(x_pows, avg_func(sims_pow, 1), var_func(sims_pow, 1),
               color=COLORS['PW'], xlabel='Oscillation Power', ax=ax2)

    leg_lines = [ax.get_lines()[0], ax2.get_lines()[0]]
    leg_labels = ['Frequency', 'Power']

    if expected is not None:
        plot_lines(x_freqs, expected, color='k', linestyle='--', ax=ax)
