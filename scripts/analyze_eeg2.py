...
def main():
    ...
    if RUN_GROUP:
        ...
        save_pickle(group_corrs, 'eeg2_group_corrs', OUTPATH)

        # Compute and save differences
        group_exp_corrs = compute_corrs_to_feature(group_avg_ts, group_avg['specparam'])
        save_pickle(group_exp_corrs, 'eeg2_group_exp_corrs', OUTPATH)
        ...
        save_pickle(group_results_peaks, 'eeg2_group_results_peaks', OUTPATH)
        ...
        save_pickle(group_alpha_corrs, 'eeg2_group_alpha_corrs', OUTPATH)

        # Compute differences between peak correlations
        group_alpha_corr_diffs = compute_diffs_to_feature(\
            group_avg_ts, group_avg_peaks['alpha_power'])
        save_pickle(group_alpha_corr_diffs, 'eeg2_group_alpha_corr_diffs', OUTPATH)
